...
def Adjust(st_degr=10):  # встать параллельно препятствию . не ставить параметр меньше 10
    global differincial, first_adjust, last_range
    ...
    if dist == -1:  # если справа ничего - встаем в прежнюю позицию и движимся
        ...
        if Look_forward(step):  # если проход достаточно широк
            ...
            rasst2 = dist
            Round(-180)
            if rasst > 0 and rasst2 > 0:
                Step(rasst - (rasst + rasst2) / 2)
                if first_adjust:
                    last_range = rasst + rasst2
                    first_adjust = False
                    differincial = 0
                elif last_range == rasst + rasst2:
                    differincial += atan((rasst - (rasst + rasst2) / 2)/step) / pi * 180
                else:
                    last_range = rasst + rasst2
                    first_adjust = True
            elif rasst > 0:
...
def Mod_LookForfard(rasst=1):
    ang = atan(DRONW / DRONH)  # угол дрона
    Round(ang / pi * 180)  # посмотреть на крайнюю левую точку
    raast2 = dist  # сохранить расстояние до препятствия
    Round(-2*ang / pi * 180)  # вернуться на прежний угол
    raast31 = dist
    Round(ang / pi * 180)
    if raast2 == -1 and raast31 == -1:  # если пусто то точно влезем
        return True  # если пусто то точно влезем
    print(raast2 * cos(ang) >= rasst + 0.2 and raast31 * cos(ang) >= rasst + 0.2)
    return raast2 * cos(ang) >= rasst + 0.2 and raast31 * cos(ang) >= rasst + 0.2
    # если расстояние больше того на которое мы двигаемся - то влезем
...
if __name__ == '__main__':  # если запущен этот файл
    ...
    while Run:  # сам цикл
        if (dist == -1 or dist >= 3) and not in_line():  # если впереди пусто и мы не на на линии старт-конец
            Round(Shortest_route() - dron_degr)  # поворачиваемся
            first_adjust = True
        if (dist == -1 or dist >= 3):  # если впереди пусто
